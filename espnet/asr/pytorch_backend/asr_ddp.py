...
class CustomConverter(object):
    ...
    def __call__(self, batch):
        ...
        if xs[0].dtype.kind == 'c':
            ...
            xs_pad = pad_list([torch.from_numpy(x).float() for x in xs], 0).to(self.dtype).cuda(self.device,
                                                                                                non_blocking=True)
        ...
        if self.reverse == False:
            ...
            ys_pad = pad_list([torch.from_numpy(
                np.flip(np.array(y[0]), 0).copy() if isinstance(y, tuple) else np.flip(y, 0).copy()).long()
                               for y in ys], self.ignore_id).cuda(self.device, non_blocking=True)
        if self.task == "asr":
